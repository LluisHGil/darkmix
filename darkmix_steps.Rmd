...
In this notebook we reproduce the example presented in section 6 of publication Hurtado-Gil et al. in prep.

Requirements:

* R version 3.5.2 (2018-12-20) -- "Eggshell Igloo" on Macintosh (or later)
* FFTW 3.3.8 (http://www.fftw.org)
* voro++ 0.4.6 (http://math.lbl.gov/voro++/)
* Compile C++ functions in Code folder as explained in the GitHub repository
...
Initialize the parameters vector and generate an initial guess of the clusters centers:
...
Visiualize again to see the new model:
...
If the model is trusted, use it estimate the number of particles per cluster:
...
```{r, message=FALSE}
maps <- c("s", "e", "d", "m") 
plot.mm3d(param, param2, clust, quad, residual.type="raw", bandwidth=1, maps=maps, fast=TRUE, col.point=1, scale="log", proj.var="Z", print=FALSE, print.data=TRUE, w=880, h=880) 
```
...
```{r, warning=FALSE}
window <- clust$domain
data_generated <- gen.pattern(param, param2, pop, window, scl=4)
```
